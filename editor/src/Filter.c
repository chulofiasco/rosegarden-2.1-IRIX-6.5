...
static void GetFiltersFromDirectory(String dirName)
{
  ...
  while ((entry = readdir(dir)) != NULL) {
    ...
    if (!file) continue;
    if (!fgets(buffer, 79, file)) { fclose(file); continue; }
    
    if (fscanf(file, "# FilterName%c", &c) != 1 || c != ':') { fclose(file); continue; }
    fscanf(file, " ");
    if (!fgets(filterName, 79, file)) { fclose(file); continue; }
    if (!filterName || !filterName[0]) { fclose(file); continue; }
...
void FilterMenuDumpStaff(Widget w, XtPointer a, XtPointer b)
{
  ...
  if (!fp) {
    fprintf(stderr, "Rosegarden: Error: Cannot create /tmp/rosebowl.tcl\n");
    End;
  }
